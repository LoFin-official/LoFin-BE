...
router.get("/", authenticate, async (req, res) => {
  try {
    ...
    if (!myProfile.connected || !myProfile.coupleId) {
      return res
        .status(400)
        .json({ message: "커플 연결이 되어 있지 않습니다." });
    }
    ...
    if (!partnerProfile) {
      return res
        .status(404)
        .json({ message: "상대방 프로필을 찾을 수 없습니다." });
    }
    ...
    res.json({
      myProfile: {
        ...
        birth: myProfile.birth
          ? myProfile.birth.toISOString().split("T")[0]
          : null,
      ...
      coupleInfo: {
        firstMeetingDate: myProfile.firstMetDate
          ? myProfile.firstMetDate.toISOString().split("T")[0]
          : null,
      },
    });
  } catch (error) {
